...
ROBOT_CONFIG2 = {
    ...
    'waypoints': [
        ...
        ([-5.9, -0.8], TurtleBot4Directions.NORTH)
    ],
    'spin_angle': 2 * math.pi,
    'nav_timeout': 30.0,
    'skip_docking': False  # 도킹 기능 활성화 (dock_status 토픽 존재함)
...
class NamespacedRobotController:
    ...
    def setup_mqtt(self):
        ...
        def on_message(client, userdata, msg):
            try:
                ...
                elif json.loads(payload).get('type') == 'human1':
                    print('Crack detected - processing location')
                    self.stop_flag = True
                    self.navigator.cancelTask()
                    self.stop_flag = False
                ...
                elif json.loads(payload).get('mapping_1') == 1:
                        self.stop_flag = True
                        self.navigator.cancelTask()
                        self.stop_flag = False
    ...
    def run_patrol_cycle(self):
        ...
        try:
            while rclpy.ok() and not self.is_stopped():
                cycle_count += 1
